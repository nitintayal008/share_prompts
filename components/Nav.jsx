...
const Nav = () => {
  ...
  return (
    <nav className='flex-between w-full md-16 pt-3'>
        ...
        <div className='sm:flex hidden'>
            {session?.user ?(
                <div className='flex gap-3 md:gap-5'>
                    <Link href="/crete-prompt" className='black_btn'>Create Post</Link>
                    <button type='button' onClick={signOut} className='outline_btn'>Sign Out</button>
                    <Link href="/profile">
                        <Image src={session?.user.image} width={37} height={37} className='rounded_full' alt='profile'/>
                    </Link>
                </div>
            ): (
            <>
        ...
        {console.log("fwd", session?.user.image)}
